...
export function KanbanBoard() {
  ...
  useEffect(() => {
    const handleDragStart = (e: DragEvent) => {
      const leadId = e.dataTransfer?.getData('text/plain')
      // dataTransfer is empty on dragstart, we set it in the card
    }
    ...
    // We track dragging via a custom approach
    // Store the dragging ID when a card starts dragging
    const handleDragStartGlobal = (e: DragEvent) => {
      // This fires before the card's onDragStart sets data
      // We'll rely on the card component instead
    }
  ...
  // Track which card is being dragged via a ref set from the card
