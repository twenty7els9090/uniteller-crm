...
function SelectTrigger({ className, size = "default", children, ...props }: React.ComponentProps<typeof SelectPrimitive.Trigger> & { size?: "sm" | "default" }) {
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "border-input data-[placeholder]:text-muted-foreground/60 flex w-full items-center justify-between gap-2 rounded-lg border bg-white px-3 py-2 text-sm shadow-xs outline-none transition-[border-color,box-shadow]",
    ...
    >
      ...
      <SelectPrimitive.Icon asChild>
        <ChevronDownIcon className="size-4 shrink-0 opacity-40" />
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          "bg-popover text-popover-foreground relative z-50 min-w-[8rem] overflow-x-hidden overflow-y-auto rounded-xl border shadow-popover",
      ...
      >
        ...
        <SelectPrimitive.Viewport className={cn("p-1", position === "popper" && "scroll-my-1")}>
          {children}
