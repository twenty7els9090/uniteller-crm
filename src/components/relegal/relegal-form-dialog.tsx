...
export function RelegalFormDialog({ open, onOpenChange, relegal, onSaved }: RelegalFormDialogProps) {
  ...
  const { register, handleSubmit, reset, setValue, watch } = useForm<RelegalFormData>({
    resolver: zodResolver(relegalSchema),
    defaultValues: { fromOrg: '', toOrg: '', action: '', manager: '' },
  })

  useEffect(() => {
    if (open && relegal) {
      reset({ id: relegal.id, fromOrg: relegal.fromOrg || '', toOrg: relegal.toOrg || '', action: relegal.action || '', manager: relegal.manager || '' })
    } else if (open) {
      reset({ id: undefined, fromOrg: '', toOrg: '', action: '', manager: '' })
    }
  }, [open, relegal, reset])
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg p-0 gap-0 max-h-[100dvh] md:my-4 my-0 rounded-b-none md:rounded-2xl bg-surface-2 border-slate-200/80 shadow-popover">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-slate-100">
          ...
          <DialogDescription className="text-zinc-500">{isEditing ? 'Измените данные' : 'Заполните информацию о смене юр.лица'}</DialogDescription>
        </DialogHeader>
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="overflow-y-auto px-6 py-5 flex-1">
            <div className="grid grid-cols-1 gap-x-5 gap-y-4">
              <div className="space-y-1.5">
                <Label htmlFor="relegal-from" className={labelClass}>С кого переключение?</Label>
                <Input id="relegal-from" placeholder="ООО «Название»" className={inputClass} {...register('fromOrg')} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="relegal-to" className={labelClass}>На кого переключение?</Label>
                <Input id="relegal-to" placeholder="ООО «Название»" className={inputClass} {...register('toOrg')} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="relegal-action" className={labelClass}>Что было сделано?</Label>
                <Textarea id="relegal-action" placeholder="Описание действий" rows={2} className={`resize-none ${inputClass}`} {...register('action')} />
              </div>
              <div className="space-y-1.5">
