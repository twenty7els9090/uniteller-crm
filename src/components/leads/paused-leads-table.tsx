...
import { getSlaDays, getSlaColorClass } from '@/lib/status'
import { Trash2, PauseCircle, Search, Phone, Mail } from 'lucide-react'

export function PausedLeadsTable() {
  ...
  const fetchLeads = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/leads?zayavka=На+паузе')
      if (res.ok) {
      ...
      <div className="hidden md:block rounded-xl border bg-card overflow-hidden card-soft">
        {filtered.length ? (
          <div className="divide-y">
            {filtered.map((lead) => {
              const days = getSlaDays(lead.statusChangedAt)
              return (
                <div
                  key={lead.id}
                  className="group flex items-center gap-4 px-4 py-3 transition-all duration-200 border-l-[3px] border-l-orange-300 bg-orange-50/20 hover:bg-orange-50/40"
                >
                  {/* Date */}
                  <div className="flex flex-col items-center justify-center w-11 h-10 rounded-md bg-orange-50 shrink-0">
                    <span className="text-sm font-bold tabular-nums leading-none text-orange-600">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {lead.comment && (
                      <p className="text-xs text-muted-foreground mt-1 truncate max-w-md">{lead.comment}</p>
                    )}
                  </div>

                  {/* Days on pause */}
                  {days > 0 && (
                    <span className={`text-xs font-medium shrink-0 ${getSlaColorClass(days)}`}>
                      {days} дн.
                    </span>
                  )}
                  ...
                  {/* Status */}
                  {lead.status && (
                    <Badge variant="outline" className="text-[10px] px-1.5 py-0 shrink-0">{lead.status}</Badge>
                  )}
          ...
          <div className="flex flex-col items-center gap-2 py-16 text-muted-foreground">
            <PauseCircle className="h-9 w-9 opacity-25" />
            <p className="text-sm">Лидов на паузе нет</p>
          </div>
        )}
        ...
        {filtered.length ? filtered.map((lead) => {
          ...
          return (
            <motion.div
              key={lead.id}
              variants={slideUp}
              className="rounded-xl border border-orange-100 bg-card p-4 space-y-2 card-soft"
            >
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground">
                  {lead.statusChangedAt ? formatDate(lead.statusChangedAt) : ''}
                  {days > 0 && (
                    <span className={`ml-2 font-medium ${getSlaColorClass(days)}`}>{days} дн.</span>
                  )}
                </span>
                {lead.status && (
                  <Badge variant="outline" className="text-[10px] px-1.5 py-0">{lead.status}</Badge>
                )}
              </div>
              <div>
                <p className="font-semibold text-sm leading-tight">{lead.organization}</p>
                {lead.partner && (
                  <Badge variant="outline" className="text-[10px] px-1.5 py-0 mt-1">{lead.partner}</Badge>
                )}
              </div>
              {lead.comment && (
                <p className="text-xs text-muted-foreground">{lead.comment}</p>
              )}
          ...
          <div className="flex items-center justify-center py-12">
            <p className="text-muted-foreground text-sm">Лидов на паузе нет</p>
          </div>
        )}
      </motion.div>
