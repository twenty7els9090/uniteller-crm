...
export async function GET(request: Request) {
  try {
    ...
    // VTB partner filter — applied FIRST and cannot be overridden
    if (user.role === 'vtb') {
      where.partner = 'ВТБ'
    }
    ...
    // partner/zayavka/manager filters from query params (but VTB partner cannot change)
    if (partner && user.role !== 'vtb') where.partner = partner
